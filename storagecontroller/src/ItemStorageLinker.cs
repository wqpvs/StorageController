...
namespace storagecontroller
{
    public class ItemStorageLinker : Item
    {
        ...
        public override void OnHeldAttackStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, ref EnumHandHandling handling)
        {
            ...
            // add a custom key?
            if (attributes.HasAttribute(posValue + "X") && blockSel == null)
            {
                BlockPos pos = attributes.GetBlockPos(posValue);
                if (pos != null)
                {
                    if (api.World.BlockAccessor.GetBlockEntity(pos) is BlockEntityStorageController blockEntityStorageController)
                    {
                        blockEntityStorageController.ClearHighlighted(entityPlayer.Player);
                        return;
                    }
                }            
            }
